...
const reddit_1 = require("@devvit/reddit");
const redis_1 = require("@devvit/redis");
const server_1 = require("@devvit/web/server");
...
const REPLY_FOOTER = "^(I'm a bot for r/LLMPhysics. Use `!define <term>` to look up a physics concept.)";
const REPLY_NOT_FOUND = 'No Wikipedia article found for "{term}".';
const REPLY_OFF_TOPIC = 'Sorry, "{term}" doesn\'t look like a science topic I cover.';
const REPLY_ERROR = "Sorry, I couldn't reach Wikipedia right now. Try again in a moment.";
// ---------------------------------------------------------------------------
// Settings helpers
// ---------------------------------------------------------------------------
async function getBotUsername() {
    const val = await server_1.settings.get('botUsername');
    return (val ?? 'llmphysics-bot').toLowerCase();
}
async function getAllowedKeywords() {
    const val = await server_1.settings.get('allowedCategoryKeywords');
    return (val ?? '').split(',').map((k) => k.trim().toLowerCase()).filter(Boolean);
}
async function getBlockedTerms() {
    const val = await server_1.settings.get('blockedTerms');
    return (val ?? '').split(',').map((t) => t.trim().toLowerCase()).filter(Boolean);
}
async function getDigestCron() {
    const val = await server_1.settings.get('digestCron');
    return val ?? '0 0 * * 0';
}
async function getDigestWikiPage() {
    const val = await server_1.settings.get('digestWikiPage');
    return val ?? 'mod-digest';
}
async function getDigestPostTitle() {
    const val = await server_1.settings.get('digestPostTitle');
    return val ?? 'Weekly Mod Digest';
}
async function getSummarySentences() {
    const val = await server_1.settings.get('summarySentences');
    return val ?? 3;
}
// ---------------------------------------------------------------------------
// Command parsing
...
function extractTerm(afterCommand) {
...
function parseCommand(body, botUsername) {
...
function isAllowedTopic(categories, keywords) {
...
async function ensureDigestScheduled() {
    ...
    const current = await redis_1.redis.get(DIGEST_CRON_KEY);
    if (current === desired) {
        console.log(`digest: cron already in sync ("${desired}"), no reschedule`);
        return;
    }
    const existingId = await redis_1.redis.get(DIGEST_JOB_ID_KEY);
    if (existingId) {
        try {
            await server_1.scheduler.cancelJob(existingId);
    ...
    try {
        const jobId = await server_1.scheduler.runJob({
            name: 'mod-digest',
            cron: desired,
        });
        await redis_1.redis.set(DIGEST_JOB_ID_KEY, jobId);
        await redis_1.redis.set(DIGEST_CRON_KEY, desired);
...
app.post('/internal/triggers/on-comment-submit', async (c) => {
    ...
    if (!parsed) {
        console.log(`define: comment ${comment.id} contains "${COMMAND}" but no valid invocation; skipping`);
        return c.json({ status: 'ok' });
    }
    ...
    if (blockedTerms.length > 0 &&
        ...
        await replyWith(comment.id, renderTemplate(REPLY_OFF_TOPIC, { term }) + footer);
        return c.json({ status: 'ok' });
    }
    let replyText;
    try {
...
app.post('/internal/triggers/on-app-upgrade', async (c) => {
    ...
    try {
        await redis_1.redis.del(DIGEST_JOB_ID_KEY);
    }
    catch {
        /* non-fatal */
    }
...
app.post('/internal/scheduler/mod-digest', async (c) => {
    await c.req.json();
    const { subredditName } = server_1.context;
    if (!subredditName) {
    ...
    let content;
    try {
        const wiki = await reddit_1.reddit.getWikiPage(subredditName, page);
    ...
    let postId;
    try {
        const post = await reddit_1.reddit.submitPost({
            subredditName,
            title,
            text: content,
        });
    ...
    try {
        const post = await reddit_1.reddit.getPostById(postId);
    ...
    try {
        await reddit_1.reddit.updateWikiPage({
            subredditName,
            page,
